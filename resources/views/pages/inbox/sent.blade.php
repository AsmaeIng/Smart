      ...
      <div id="sidebar-list" class="sidebar-menu list-group position-relative animate fadeLeft">
        <div class="sidebar-list-padding app-sidebar sidenav" id="email-sidenav">
          <ul class="email-list display-grid">
            ...
            <li class="active"><a href="{{asset('sent')}}" class="text-sub"><i class="material-icons mr-2"> send </i> Sent</a></li>
          ...
          <div class="email-header">
            <div class="left-icons">
              ...
              <span class="action-icons">
                <button ng-click="refreshSent()"><i class="material-icons">refresh</i></button>
          ...
          <div class="collection email-collection">
				...
				@foreach($sents as $sent)
				 <div class="email-brief-info collection-item animate fadeUp delay-1">
					  <div class="list-left">
						<label>
						  <input type="checkbox" name="inbox_ids" ng-click="fillArrayWithMailsToDelete(<?php echo $sent->inbox_id; ?>)" />
						  <span></span>
						</label>
						<div class="favorite">
							@if($sent->starred == 1)
								<a name="inbox_ids[]" ng-click="fillArrayWithMailsToStarred(<?php echo $sent->inbox_id; ?>)"><i class="material-icons amber-text">star_border</i></a>
							@elseif($sent->starred == 0) 
								<a name="inbox_ids[]" ng-click="fillArrayWithMailsToStarred(<?php echo $sent->inbox_id; ?>)"><i class="material-icons">star_border</i></a>							
							@endif
						</div>
						<div class="email-label">							
							@if($sent->important == 1)
								<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $sent->inbox_id; ?>)"><i class="material-icons amber-text">label_outline</i></a>
							@elseif($sent->important == 0) 
								<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $sent->inbox_id; ?>)"><i class="material-icons">label_outline</i></a>
							@endif
						</div>
					  </div>
					  <a class="list-content" href="app-email/content/{{$sent->inbox_id}}"> <!-- /sent/{{$sent->inbox_id}}-->
						<div class="list-title-area">
						  <div class="user-media">
							<img src="/{{$sent->path}}" alt="" class="circle z-depth-2 responsive-img avtar">
							<div class="list-title">{{$sent->username}} {{$sent->lastname}} </div>
						  </div>
						...
						<div class="list-desc">{{$sent->subject}} : {{$sent->message}}</div>
					  </a>
					  <div class="list-right">
						<div class="list-date"> {{$sent->created_at->diffForHumans()}} </div>
					  </div>
					</div>
					  @endforeach

      ...
      <div id="sidebar-list" class="sidebar-menu list-group position-relative animate fadeLeft">
        <div class="sidebar-list-padding app-sidebar sidenav" id="email-sidenav">
          <ul class="email-list display-grid">
            ...
            <li ><a href="{{asset('app-email')}}" class="text-sub"><i class="material-icons mr-2"> mail_outline </i> Inbox Inbox&nbsp;&nbsp;@if($numberOfMessages>0)<span class="badge chip lighten-5 red red-text">{{$numberOfMessages}}</span>@endif</a>
            </li>
            ...
            <li class="active"><a href="{{asset('starred')}}"class="text-sub"><i class="material-icons mr-2"> star_border </i> Starred</a></li>
      ...
      <div class="card card card-default scrollspy border-radius-6 fixed-width">
        <div class="card-content p-0 pb-2">
          ...
          <div class="collection email-collection">
				...
				@foreach($getStarred as $starred)
				 <div class="email-brief-info collection-item animate fadeUp delay-1">
              <div class="list-left">
                <label>
                  <input type="checkbox"  type="checkbox" name="inbox_ids[]" ng-click="fillArrayWithMailsToDelete(<?php echo $starred->inbox_id; ?>)" />
                  <span></span>
                </label>
                <div class="favorite">
                  <a  name="inbox_ids[]" ng-click="fillArrayWithMailsToStarred(<?php echo $starred->inbox_id; ?>)"><i class="material-icons amber-text">star_border</i></a>
                </div>
                <div class="email-label">
					@if($starred->important == 1)
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $starred->inbox_id; ?>)"><i class="material-icons amber-text">label_outline</i></a>
					@elseif($starred->important == 0) 
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $starred->inbox_id; ?>)"><i class="material-icons">label_outline</i></a>
					@endif
                </div>
              </div>
             <a class="list-content" href="app-email/content/{{$starred->inbox_id}}"> 
                <div class="list-title-area">
                  <div class="user-media">
                    <img src="/{{$starred->path}}" alt="" class="circle z-depth-2 responsive-img avtar">
                    <div class="list-title">{{$starred->username}} {{$starred->lastname}}</div>
					 @if($starred->from_user_id == Auth::user()->id)
						  You
						@else
						  {{$starred->username}} {{$starred->lastname}}
						@endif
                  </div>
                ...
                <div class="list-desc">{{$starred->subject}} : {{$starred->message}}</div>
              </a>
              <div class="list-right">
                <div class="list-date"> {{$starred->updated_at->diffForHumans()}} </div>
              </div>
            </div>
					  @endforeach

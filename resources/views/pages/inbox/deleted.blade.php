      ...
      <div id="sidebar-list" class="sidebar-menu list-group position-relative animate fadeLeft">
        <div class="sidebar-list-padding app-sidebar sidenav" id="email-sidenav">
          <ul class="email-list display-grid">
            ...
            <li ><a href="{{asset('app-email')}}" class="text-sub"><i class="material-icons mr-2"> mail_outline </i>  Inbox&nbsp;&nbsp;@if($numberOfMessages>0)<span class="badge chip lighten-5 red red-text">{{$numberOfMessages}}</span>@endif</a>
            </li>
            <li ><a href="{{asset('sent')}}" class="text-sub"><i class="material-icons mr-2"> send </i> Sent</a></li>
			<li class="active"><a href="{{asset('deleted')}}" class="text-sub"><i class="material-icons mr-2"> delete </i> Trash</a></li>
      ...
      <div class="card card card-default scrollspy border-radius-6 fixed-width">
        <div class="card-content p-0 pb-2">
          <div class="email-header">
            <div class="left-icons">
              ...
              <span class="action-icons">
                <button ng-click="refreshDeleted()"><i class="material-icons">refresh</i></button>
               <a href="#sendEmail"> <i class="material-icons">mail_outline</i></a>
                <i class="material-icons">folder_open</i>
                <i class="material-icons">info_outline</i>
				<button ng-click="starredSentMail()" ng-if="checkForStarred"><i class="material-icons">star_border</i></button>
				<button ng-click="importantSentMail()" ng-if="checkForImportant"><i class="material-icons">label_outline</i></button>
                <button ng-click="removeMail()" ng-if="checkForDelete"><i class="material-icons delete-mails">delete</i></button>
              </span>
            </div>
          ...
          <div class="collection email-collection">
              ...
              <div class="list-left">
                <label>
                  <input type="checkbox"  type="checkbox" name="inbox_ids[]" ng-click="fillArrayWithMailsToDelete(<?php echo $deleted_mail->inbox_id; ?>)" />
                  <span></span>
                </label>
                <div class="favorite">
					@if($deleted_mail->starred == 1)
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToStarred(<?php echo $deleted_mail->inbox_id; ?>)"><i class="material-icons amber-text">star_border</i></a>
					@elseif($deleted_mail->starred == 0) 
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToStarred(<?php echo $deleted_mail->inbox_id; ?>)"><i class="material-icons">star_border</i></a>							
					@endif
				</div>
                <div class="email-label">
					@if($deleted_mail->important == 1)
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $deleted_mail->inbox_id; ?>)"><i class="material-icons amber-text">label_outline</i></a>
					@elseif($deleted_mail->important == 0) 
						<a name="inbox_ids[]" ng-click="fillArrayWithMailsToImportant(<?php echo $deleted_mail->inbox_id; ?>)"><i class="material-icons">label_outline</i></a>
					@endif
                </div>
              </div>
              <a class="list-content" href="#">
                <div class="list-title-area">
                  <div class="user-media">
                    <img src="/{{$deleted_mail->path}}" alt="" class="circle z-depth-2 responsive-img avtar">
                    <div class="list-title">{{$deleted_mail->username}} {{$deleted_mail->lastname}}</div>
					 @if($deleted_mail->from_user_id == Auth::user()->id)
						  You
						@else
						  {{$deleted_mail->username}} {{$deleted_mail->lastname}}
						@endif
                  </div>
                ...
                <div class="list-desc">{{$deleted_mail->subject}} : {{$deleted_mail->message}}</div>
              </a>
              <div class="list-right">
                <div class="list-date"> {{$deleted_mail->updated_at->diffForHumans()}} </div>
              </div>
            </div>
					  @endforeach
...
<div class="email-compose-sidebar" id="sendEmail">
  <div class="card quill-wrapper">
    <div class="card-content pt-0">
      ...
      <!-- form start -->
      <form class="edit-email-item mt-10 mb-10" method="POST" action="/sendMail" >
	 @csrf <!-- {{ csrf_field() }} -->
        ...
        <div class="input-field">
          <div class="snow-container mt-2">
            ...
            <div class="compose-quill-toolbar" name="message">
      ...
      <div class="card-action pl-0 pr-0 right-align">
        <button type="reset" class="btn-small waves-effect waves-light cancel-email-item mr-1 waves-light">
          <i class="material-icons left">close</i>
          <span>Cancel</span>
        </button>
        <button class="btn-small waves-effect waves-light waves-light" type="submit" name="action">
